...
func goschedImpl(gp *g, preempted bool) {
	...
	for i := 0; i < len(allp); i++ {
		...
		if pp == allp[i] {
			// if allp[(i + 1) % 2].status != _Pidle {
			// 	runqput(allp[(i + 1) % 2], gp, false)
			// 	break
			// } else {
			// 	// runqput(pp, gp, false)
			// 	lock(&sched.lock)
			...
			if pp.shortRunning == 0 && i == half - 1 {
				if allp[0].status == _Prunning {
					runqput(allp[0], gp, false)
					break
				}  
			}

			if allp[(i + 1) % 4].status == _Prunning {
				runqput(allp[(i+1) % 4], gp, false)
				break
			} else {
					lock(&sched.lock)
					globrunqput(gp)
...
func procresize(nprocs int32) *p {
    ...
    for i := int32(0); i < nprocs; i++ {
        ...
        if i < half {
            pp.shortRunning = 0 // First half of Ps are "short-running"
			pp.preemptlimit = 10 * 1000 * 1000
        } else {
            pp.shortRunning = 1
			pp.preemptlimit = 10 * 1000 * 1000
        }
...
var zero uintptr // Global zero-initialized variable
...
func checkdead() {
	...
	// forEachG(func(gp *g) {
    //     if readgstatus(gp) == _Gwaiting && gp.waiting != nil && gp.waiting.c != nil {

    //         // Check if the goroutine is waiting on a receive operation
    //         if gp.waitreason == waitReasonChanReceive {
	// 			casgstatus(gp, _Gwaiting, _Grunnable)

	// 			print("Goroutine ", gp.goid, " is waiting on a channel\n")
	// 			// var tempValue = 42
	// 			// gp.waiting.elem = unsafe.Pointer(&tempValue)
	// 			// newGp := newproc(&funcPtr)

	// 			// Set the status of the new goroutine to "runnable"
	// 			// newGp.status = _Grunnable
	...
	// 			// gp.waiting = mysg
	// 			// gp.param = nil
	// 			// gp.waiting.c.sendq.enqueue(mysg)

	// 			// Create or reference the channel (chanType)
	// 			// unlock(gp.waiting.c)

	// 			// Add the new goroutine to the sendqueue of the channel
	// 			var newlist gList
	// 			newlist.head = guintptr(unsafe.Pointer(gp)) 
	// 			// lock(&gp.waiting.c.lock)
	// 			// send(gp.waiting.c, gp.waiting, unsafe.Pointer(&zero), func() { unlock(&gp.waiting.c.lock) }, 3)
	// 			// casgstatus(gp, _Gwaiting, _Grunnable)
	// 			injectglist(&newlist)
	// 			// Step 2: Put goroutine into the current processor's run queue
	// 			// pp := gp.m.p.ptr() // Get the current P
	// 			// print(pp.id)
	// 			// print(nil)
	// 			// runqput(pp, gp, false)
	...
	// forEachG(func(gp *g) {
	// 	// print("GP ID ", gp.goid, " on channel receive\n")
	// 	// print("Status ", readgstatus(gp), "\n")
	// 	// print("Wait Status ", gp.waiting, "\n")
	// 	// print("Wait reason ", gp.waitreason, " on channel receive\n")
	// 	// print("Wait element ", gp.waiting.c, " on channel receive\n")


	// 	if readgstatus(gp) == _Gwaiting {
	// 		// print(gp.waiting != nil)
	// 		// print(gp.waiting.elem != nil)

	// 		if gp.waiting != nil && gp.waiting.c != nil {
	// 			print("If block ", gp.goid, " on channel receive\n")
	// 			    // Step 2: Only change the state if it is in _Gwaiting, which is a valid state for wakeup.
	// 				// if readgstatus(gp) == _Gwaiting {
	// 					// Step 3: Create a new gList to add the goroutine to the scheduler
	// 					ready(gp, 0, true)
	// 					print("Waking up the goroutine ", gp.goid, " successfully!\n")
	// 				// } else {
	// 				// 	print("Goroutine ", gp.goid, " is not in _Gwaiting state, current status: ", readgstatus(gp), "\n")
	// 				// }
	// 				return
